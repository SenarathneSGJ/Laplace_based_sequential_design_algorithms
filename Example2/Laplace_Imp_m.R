Lp_Imp_m <- function(datat,int.theta1,theta_MH,W_MH,model,N,lwr,upr,log_postf){
  post1 <- Laplace_approx(data_all=datat,theta=int.theta1,model=model,log_postf=log_postf,lwr=lwr,upr=upr)
  mu_post <- post1$par
  Hess1 <- post1$hessian
  ...
  if(model==1){
    log.prior<- dmvnorm(x=lp_post,mean=rep(0,length(mu_post)),sigma=diag(c(rep(SD^2,8),1.5^2)),log=TRUE)
  }else{
    log.prior<- dmvnorm(x=lp_post,mean=rep(0,length(mu_post)),sigma=diag(rep(SD^2,8)),log=TRUE)
  }
