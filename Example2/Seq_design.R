...
Seq_d <- function(T1,theta,W,model){
  ...
  Hess_prev1 <- solve(diag(c(rep(SD,8),1.5)^2))
  Hess_prev2  <- solve(diag(rep(SD,8)^2))  
  ...
  for(t1 in 0:(T1-1))
  {
    ...
    post1 <- Laplace_approx(data_all=datat,theta=int.theta1,model=1,log_postf=log_post,lwr=c(apply(prior[,1:9],2,min)),upr=c(apply(prior[,1:9],2,max)))
    mu_post1 <- post1$par
    int.theta1 <- mu_post1
    Hess1 <- post1$hessian
    ...
    post2 <- Laplace_approx(data_all=datat,theta=int.theta2,model=2,log_postf=log_post,lwr=c(apply(prior[,10:17],2,min)),upr=c(apply(prior[,10:17],2,max)))
    mu_post2 <- post2$par
    int.theta2 <- mu_post2
    Hess2 <- post2$hessian
    ...
    p.theta1 <- dmvnorm(mu_post1,mean=rep(0,9),sigma=diag(c(rep(SD^2,8),1.5^2)),log=T)
    log.w2 <- sum(likelihood_m(data=datat,para=data.frame(t(mu_post2)),model=2)) 
    p.theta2 <- dmvnorm(mu_post2,mean=rep(0,8),sigma=diag(rep(SD^2,8)),log=T)
    
    LogZs.m1 <- (9/2)*log(2*pi)+0.5*log(det(sigma_post1))+log.w1+p.theta1
    LogZs.m2 <- (8/2)*log(2*pi)+0.5*log(det(sigma_post2))+log.w2+p.theta2
   
    LogZs <- c(LogZs.m1,LogZs.m2)
	  LogZs[is.na(LogZs)] <- (-200)
  ...
  out.list <- list(datat,mu_post_all,post_sig,post_model_probs_all,utility,Log_det,ute_t)
  return(out.list)
}