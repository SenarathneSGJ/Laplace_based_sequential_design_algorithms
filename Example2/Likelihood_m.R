...
likelihood_m=function(data,para,model) 
{
  ...
  if(model==1){
    ...
    {
      #alpha_Fr[i]=uniroot(fn2, lower=-1e+10, upper=1e+10,k=tau[i])$root
      alpha_Fr[i]=multiroot(fn2, start=10,k=tau[i],maxiter=100000)$root
      if(tau[i]>0 & alpha_Fr[i] < 0){
        alpha_Fr[i]=0.01
      }
    ...
    frankC[is.na(frankC) | frankC==Inf | frankC>pi1]<-pi1[ is.na(frankC) | frankC==Inf | frankC>pi1]
    frankC[frankC>pi2]<-pi2[frankC>pi2]
    frankC[frankC<=0] <- (1e-100)
