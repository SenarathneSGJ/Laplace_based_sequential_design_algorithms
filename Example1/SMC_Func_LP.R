...
SMC_Func_LP=function(T1,theta,W,model)
{  
  datat <- data.frame()
  ...
  for(t1 in 0:(T1-1))
  {
    ...
    {
      out1 <- Lp_Imp_m1(datat=datat,int.theta1=int.theta1,N=N,log_postf=log_post1,lwr=c(apply(prior[,1:4],2,min)),upr=c(apply(prior[,1:4],2,max)))
      theta[,1:4] <- out1[[1]]
      int.theta1 <- out1[[2]]
      W[,1] <- rep(1,N)/N     
    }  
    ...
    {
      out2 <- Lp_Imp_m2(datat=datat,int.theta1=int.theta2,N=N,log_postf=log_post2,lwr=c(apply(prior[,5:9],2,min)),upr=c(apply(prior[,5:9],2,max)))
      theta[,5:9] <- out2[[1]]
      int.theta2 <- out2[[2]]
      W[,2] <- rep(1,N)/N     
    }  
