Lp_Imp_m1 <- function(datat,int.theta1,N,lwr,upr,log_postf){
  ...
  log.prior<- dmvnorm(x=lp_post1,mean=rep(0,4),sigma=diag(rep(100,4)),log=TRUE)
  log.like <- rowSums(likelihood_m1(datat,para=lp_post1))
