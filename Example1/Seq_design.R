...
Seq_d <- function(T1,theta,W,model)
{
  ...
  Hess_prev1 <- solve(diag(rep(100,4)))
  Hess_prev2  <- solve(diag(rep(100,5)))  
  ...
  for(t1 in 0:(T1-1))
  {
    ...
    post1 <- Laplace_approx(data_all=datat,theta=int.theta1,log_postf=log_post1,lwr=c(apply(prior[,1:4],2,min)),upr=c(apply(prior[,1:4],2,max)))
    mu_post1 <- post1$par
    Hess1 <- post1$hessian
    ...
    post2 <- Laplace_approx(data_all=datat,theta=int.theta2,log_postf=log_post2,lwr=c(apply(prior[,5:9],2,min)),upr=c(apply(prior[,5:9],2,max)))
    mu_post2 <- post2$par
    Hess2 <- post2$hessian
    ...
    log.w1 <- sum(likelihood_m1(data=datat,para=data.frame(t(mu_post1)))) 
    p.theta1 <- dmvnorm(mu_post1,mean=rep(0,4),sigma=diag(rep(100,4)),log=T)
    log.w2 <- sum(likelihood_m2(data=datat,para=data.frame(t(mu_post2)))) 
    p.theta2 <- dmvnorm(mu_post2,mean=rep(0,5),sigma=diag(rep(100,5)),log=T)
