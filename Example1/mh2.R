mh2=function(theta,N,W,datat,R)
{
  ...
  for (m in 1:R)
  {
    ...
    log_prior_theta_j <- dmvnorm(theta,mean=c(0,0,0,0,0),sigma=diag(rep(100,5)),log=TRUE) 
    log_prior_theta_prop<- dmvnorm(theta_prop,mean=c(0,0,0,0,0),sigma=diag(rep(100,5)),log=TRUE)  
