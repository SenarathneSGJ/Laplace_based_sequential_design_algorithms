...
{  
  ...
  Y=data.frame(y1=c(0,1)) # select four outcomes (Y1,Y2)=(00,01,10,11)
  ...
  for(j in 1:2) 
  {
    ...
    log_w <- c(likelihood_Func(data=data_select[j,],para=theta))  # likelihood_Func returns log_likelihood values
    w=exp(log_w)
